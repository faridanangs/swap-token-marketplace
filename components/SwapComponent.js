...
//   console.log(outputValue, "outputval")
...
const SwapComponent = () => {
  ...
  function populateOutputValue() {
    ...
    try {
      if (srcToken !== ETH && destToken !== ETH) {
        ...
        const outValue = toWei(inputValue, 14); // Ensure to use 18 decimals
        setOutputValue(outValue);
      } else if (srcToken !== ETH && destToken === ETH) {
        const outValue = toEth(toWei(inputValue, 14)); // Use proper conversion
        setOutputValue(outValue);
      }
    } catch (error) {
      setOutputValue("0");
  ...
  function populateInputValue() {
    ...
    try {
      if (destToken !== ETH && srcToken !== ETH) {
        setInputValue(outputValue);
      } else if (destToken !== ETH && srcToken === ETH) {
        const outValue = toEth(outputValue); // Ensure to use 18 decimals
        setInputValue(outValue);
      } else if (srcToken !== ETH && destToken === ETH) {
        const outValue = toEth(toWei(outputValue, 14)); // Use proper conversion
        setInputValue(outValue);
      }
    } catch (error) {
      setInputValue("0");
